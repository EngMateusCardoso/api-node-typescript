...
describe('Pessoas - Create', () => {
  ...
  beforeAll(async () => {
    const email = 'createpessoas@gmail.com'
  ...
  beforeAll(async () => {
    const resCidade = await testServer.post('/cidades').set({ Authorization: `Bearer ${accessToken}`}).send({ nome: 'CidadeTeste' });

    cidadeId = resCidade.body.id;
  });
  ...
  it('Cria registro', async () => {
    ...
    expect(typeof res1.body.id).toEqual('number');
  });
